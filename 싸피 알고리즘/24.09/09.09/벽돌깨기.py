...
def shoot(level, remains, now_arr):
    ...
    for col in range(W):
        ...
        while li:
            r, c, p = li.pop()
            for k in range(1, p):  # 파워 만큼 퍼지면서 깨진다.
...
for tc in range(1, T + 1):
    ...
    # 1. 최소 벽돌 개수
    # - 몇 개 남았을까? 계산해야한다. -> 2차원 리스트를 순회하면서 매 번 계산하면 너무 느리다
    # - 현재 남은 벽돌 수를 통해 해결됨!
    # 2. 현재 벽돌이 다 깨지면(남은 벽돌이 없으면) 더이상 진행할 필요가 없다!
